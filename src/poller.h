...
public:
  ...
  void RemoveEvent(int fd) {
    max_fd_ = std::max(max_fd_, fd);
    changes_.emplace_back(std::move(
        TEvent{fd, TEvent::READ | TEvent::WRITE | TEvent::RHUP, THandle{}}));
  }
  ...
  auto Sleep(TTime until) {
    struct TAwaitableSleep {
      ...
      TAwaitableSleep(TAwaitableSleep &&other)
          : poller_(other.poller_), n_(other.n_) {
...
protected:
  ...
  timespec max_duration_ts_ =
      GetMaxDuration(max_duration_); // max poll duration in timespec
};
} // namespace NNet
