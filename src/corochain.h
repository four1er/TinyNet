...
struct TValuePromise : public TValuePromiseBase<T> {
    ...
    void return_value(const T& t) { ErrorOr = t; }

    void return_value(T&& t) { ErrorOr = std::move(t); }

    void unhandled_exception() { ErrorOr = std::current_exception(); }
...
struct TValueTaskBase {
    ...
    TValueTaskBase(TValuePromise<T>& promise) : Coro(Coro.from_promise(promise)) {}
    TValueTaskBase(TValueTaskBase&& other) { *this = std::move(other); }
    ...
    bool await_ready() const { return Coro.promise().ErrorOr != std::nullopt; }

    bool done() const { return Coro.done(); }

    std::coroutine_handle<> raw() { return Coro; }

    void await_suspend(std::coroutine_handle<> caller) { Coro.promise().Caller = caller; }
...
struct TValuePromise<void> : public TValuePromiseBase<void> {
    ...
    void return_void() { ErrorOr = nullptr; }
...
inline TValueTask<void> TValuePromise<void>::get_return_object() {
    return {TValueTask<void>{*this}};
}
template <typename T>
TValueTask<T> TValuePromise<T>::get_return_object() {
    return {TValueTask<T>{*this}};
}

template <typename T>
TFinalAwaiter<T> TValuePromiseBase<T>::final_suspend() noexcept {
    return {};
}
...
TFuture<T> Any(std::vector<TFuture<T>>&& futures) {
    ...
    co_return std::find_if(all.begin(), all.end(), [](auto& f) {
        return f.done();
    })->await_resume();
}
